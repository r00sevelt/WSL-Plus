...
## 云端资源红线（2026-08-25 硬性——违反=事故）
```
- 云端动作（推送/合并/取消/轮询/检查）：只允许在用户逐次明确指令后执行
- 合并上云 = 用户说"合并/上云"才做；"做完"的判定权永远在用户
- 开发代码一律 feature/* 分支（零云消耗）；dev/master 推送 = 用户指令事件
- 预算纪律: Windows runner x2 计费；每次误轮询/误合并 = 记入审计并停止
```
